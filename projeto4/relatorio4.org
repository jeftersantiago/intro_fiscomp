...
* Tarefa E
  ...
  #+caption: Condição para seção de Poincaré adicionada na estrutura de repetição da rotina (\ref{diagramaFase}).
  #+caption: Os \( \theta \) e \( \omega \) são foram escritos no arquivo =./saida-secao-poincare.dat=. 
  #+begin_src fortran :exports code
